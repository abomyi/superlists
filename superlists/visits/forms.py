...
from visits.models import VisitModel, Demand, Department


class VisitForm(forms.ModelForm):
    company = forms.CharField(label='拜訪機構')
    visitDate = forms.DateField(label='拜訪日期')
    visitTime = forms.TimeField(label='拜訪時間')
    teacher = forms.CharField(label='執行教師')
    ...
    nextDate = forms.DateField(label='下次拜訪日', required=False)
    entrust_department = forms.ModelMultipleChoiceField(label='委託科系', queryset=Department.objects.all(), widget=forms.CheckboxSelectMultiple, required=False)
    entrust_teacher = forms.CharField(label='委託教師', required=False)
    other = forms.CharField(label='其他建議', widget=Textarea(attrs={'rows':'5', 'cols':'20'}))
